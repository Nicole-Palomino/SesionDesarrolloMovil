...
class AuthRepositoryImp (val auth: FirebaseAuth, val database: FirebaseFirestore, val appPreferences: SharedPreferences, val gson: Gson): AuthRepository {
    override fun registerUsuario(
        ...
        auth.createUserWithEmailAndPassword(correo, contrasena)
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    usuario.uid = it.result.user?.uid ?: ""
    ...
    override fun actualizarUsuario(usuario: Usuario, result: (UiState<String>) -> Unit) {
        ...
        val document = database.collection(FireStoreCollection.USUARIO).document(usuario.uid)
        document
            .set(usuario)
